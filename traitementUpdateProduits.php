...
if(empty($_FILES['image']['tmp_name'])) {
 ...
 $req = $bdd->prepare('UPDATE `projetPro`.`produits` SET `nom` = :nom, `prix` = :prix WHERE `produits`.`id` = :id_produits');
 $req->execute(array(
   'nom' => $nom,
   'prix' => $prix,
   'id_produits' => $idProduits
    ...
    if ($image_sizes[0] > $maxwidth OR $image_sizes[1] > $maxheight) $erreur = "Image trop grande";
     ...
     // On modifie une entrée dans la table plats
     $req = $bdd->prepare('UPDATE `projetPro`.`produits` SET `nom` = :nom, `prix` = :prix, `image` = :image WHERE `produits`.`id` = :id_produits');
     $req->execute(array(
     	'nom' => $nom,
     	'prix' => $prix,
      'image' => $image,
